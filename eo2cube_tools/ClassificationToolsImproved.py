# ClassificationToolsCustomized.py
"""
...
NOT THE ORIGINAL, THIS IS A TWEAKED VERSION!
"""
...
def randomforest_train(train_shps, train_field, data_func, data_func_params={},
    ...
    def rasterize(convert_to_rast, out_rast, meta, field):
        ...
        '''Function for rasterising a Geopandas dataframe using Rasterio'''

        with rasterio.open(out_rast, 'w', **meta) as out:
        
            out_arr = np.zeros(shape=(meta['height'], meta['width'])) # CHANGED - original: both 'height'
    ...
    for train_shp in train_shps:
        ...
        # Open vector of training points with geopandas (original says GDAL)
        #train_shp = train_shps[0]   # COMMENTED THIS OUT because why would you loop but then always choose the first file?
        ...
        if training_gpd[train_field].dtype != np.int64 and not train_reclass:
            ...
            training_gpd_latlong = training_gpd.to_crs('epsg:4326') # CHANGED that to not-deprecated version of crs statement
            ...
            training_pixels = rasterize(convert_to_rast=training_gpd_latlong, #  CHANGED to ...latlong, reprojected gpd object should be used
    ...
    if len(training_labels_list) > 0: # DEDENTED THIS PART, it has to be done after samples were collected from ALL shapefiles
...
def randomforest_classify(classifier, analysis_data, classification_output, class_prob=False):
    ...
    proj = analysis_data.crs #.wkt # REMOVED this, as there was no wkt string

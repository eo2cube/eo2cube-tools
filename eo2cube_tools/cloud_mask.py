...
def ssmask_v3(dataset):
    ...
    dataset: xr.DataArray
        A xr.DataArray with variable "mask" of values 0, 6, 9, 11, or nan (0 = land, 6 = water, 9 = cloud, 11 = snow, nan = defect scene).
    """
    ...
    if any([defectr,defectg,defectb]) == True:
        dataset = dataset.assign(mask = (dataset.blue*0)*np.nan)
        return dataset['mask']
    ...
    dataset = dataset.assign(
        ...
        NDSI_CI = (dataset["MNDWI"] - dataset["NDCI"])
    )
    
    ### Check for Water ###
    
    water_condition1 = np.logical_and(dataset.water_vapour < 1000,dataset.red_edge3 < 100)
    water_condition2 = np.logical_and(dataset.MNDWI > 0,np.logical_and(dataset.NDVI < 0.05,dataset.NDVI > -0.05))
    water_condition3 = np.logical_and(dataset.MNDWI > 0,dataset.water_vapour < 1000)
    water_condition123 = np.logical_or(np.logical_or(water_condition1, water_condition2),water_condition3)
    dataset['water'] = xr.where(water_condition123,1,0)

    if round(dataset.water.mean().values.flatten()[0],5) > 0:
        water_presence = True
    else:
        water_presence = False
        
    if water_presence == False:
        dataset['water'] = dataset.blue*0
        
    ### Check for snow ###
    dataset['snow_presence'] = xr.where(np.logical_and(np.logical_and(dataset.red_edge3 > 4000, dataset.NDSI_CI > 0.5),dataset.MNDWI > 0.4),1,0)
    if round(dataset.snow_presence.mean().values.flatten()[0],5) > 0:
        snow_presence = True
    else:
        snow_presence = False
    ...
    if snow_presence == True:
        snow_condition1 = np.logical_and(dataset["green"] > 4000, dataset["nir"] > 4000)
        ...
        dataset = dataset.assign(snow = dataset.blue*0)
    
    ### For Histogram-based Cloud Masking, all snow and water pixels are excluded before calculating histogram ###      
    ...
    ndci_arr = dataset.where(np.logical_and(dataset.snow == 0, dataset.water == 0)).NDCI.values.flatten()
    ndci_arr = ndci_arr[~np.isnan(ndci_arr)]
    vapour_arr = dataset.where(np.logical_and(dataset.snow == 0, dataset.water == 0)).water_vapour.values.flatten()
    ...
    NDCI_std = np.std(ndci_arr) # NDCI standard deviation
    ...
    cloudClass_condition = np.logical_and(dataset.water == 0, dataset.snow == 0)
    
    # Classified Scene: All Clouds
    if NDCI_min >= -0.1:
        dataset = dataset.assign(cloud = xr.where(dataset.NDSI_CI < 0.2, 1.0, 0.0))
    ...
    # Classified Scene: Mostly clouds
        ...
        dataset = dataset.assign(cloud = xr.where(np.logical_and(cloudClass_condition,condition), 1.0, 0.0)) # assign cloud variable in the dataset
    ...
    # if clouds are dominant in scene, there should be only one peak in the histogram
        ...
        dataset = dataset.assign(cloud = xr.where(np.logical_and(cloudClass_condition,condition), 1.0, 0.0))
    ...
    # Classified Scene: Partly clouds
        ...
        cloud_ndci = dataset.where(cloudClass_condition).NDCI.values.flatten()
        cloud_ndci = cloud_ndci[~np.isnan(cloud_ndci)]
        counts, bin_edges = np.histogram(cloud_ndci, bins=30)
        if npeak <= 1:
            peak = np.argmax(counts)
            mode = (bin_edges[peak] + bin_edges[peak+1])/2 # check dominant NDCI value
            par = stats.percentileofscore(cloud_ndci, mode) # get quantile for dominant NDCI value
            ...
            NDCI_thres = np.quantile(cloud_ndci,refine) # calculate the NDCI threshold
        ...
        condition1 = np.logical_and(dataset.NDCI > NDCI_thres_adjust, dataset.MNDWI < 0)
        condition2 = np.logical_and(dataset.NDCI > NDCI_thres_adjust, dataset.NDSI_CI < 0.3)
        condition3 = dataset.NDVI < 0.2
        condition = np.logical_and(np.logical_or(condition1,condition2),condition3)
        dataset = dataset.assign(cloud = xr.where(condition, 1.0, 0.0)) # add cloud variable in the dataset
    
    if snow_presence == True:
        dataset["snow"] = dataset["snow"].rolling(latitude=2, longitude=2, center=True).min()
        dataset["snow"] = dataset["snow"].rolling(latitude=2, longitude=2, center=True).max()
        dataset["snow_buffer"] = dataset["snow"].rolling(latitude=2, longitude=2, center=True).max()
        dataset["snow_buffer"] = dataset["snow_buffer"].fillna(0)
        ...
        dataset["cloud"] = dataset.cloud2
        dataset["snow"] = dataset["snow"].fillna(0)
        
    dataset["water"] = dataset["water"].where(np.logical_and(dataset.snow == 0,dataset.cloud == 0), 0)
    dataset["cloud"] = dataset["cloud"].where(dataset.snow == 0, 0)
    dataset["water"] = dataset["water"].fillna(0)
    dataset["cloud"] = dataset["cloud"].fillna(0)
    dataset["snow"] = dataset["snow"].fillna(0)
    
    # Cloud Smoothing
    dataset["cloud_fill"] = dataset["cloud"].rolling(latitude=3, longitude=3, center=True).min()
    dataset["cloud_fill"] = dataset["cloud_fill"].rolling(latitude=3, longitude=3, center=True).max()
    dataset["cloud_fill"] = dataset["cloud_fill"].fillna(value = dataset.cloud)    
    dataset["cloud"] = dataset["cloud_fill"]
    
    # Finalize Class
    dataset["mask"] = dataset['snow']*11 + dataset['water']*6 + dataset['cloud']*9 
     
    return dataset["mask"]

def ssmask_v2(dataset,strength=0.4):
    """
    Adding cloud variable to detect cloud cover in the single scene.
    Description
    ----------
...
def ssmask_v1(dataset,strength=0.4):
    """
    Adding cloud variable to detect cloud cover in the single scene.
    Description
    ----------
...
def s2cmask_v3(ds, strength=0.4):
    """
    ...
    """
    collection = []
    [collection.append(ssmask_v3(ds.sel(time = ts), strength=strength)) for ts in ds.time.to_index()]
    maskds = xr.concat(collection, dim = "time")
    return maskds
    
def s2cmask_v2(ds, strength=0.4):
    """
    Adding cloud variable to detect cloud cover in multiple scenes.
    Description
    ----------
    ...
    """
    collection = []
    [collection.append(ssmask_v2(ds.sel(time = ts), strength=strength)) for ts in ds.time.to_index()]
    maskds = xr.concat(collection, dim = "time")
    return maskds

def s2cmask_v1(ds, strength=0.4):
    """
    Adding cloud variable to detect cloud cover in multiple scenes.
    Description
    ----------
    ...
    """
    collection = []
    [collection.append(ssmask_v1(ds.sel(time = ts), strength=strength)) for ts in ds.time.to_index()]
    maskds = xr.concat(collection, dim = "time")
    return maskds